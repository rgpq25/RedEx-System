			...
			>
				...
				<CardContent className="w-full flex flex-col items-center gap-6 *:w-full overflow-hidden">
					...
					{selectedOperation === Operacion.Aeropuertos && (
						<Aeropuertos aeropuertos={aeropuertos} onClicks={onClicksAeropuerto} />
					)}
								...
								envios
									...
									.map((envio) => (
										...
										>
											...
											<CardContent>
												<Muted>
													Fecha de recepción: {formatDateShort(envio.fechaRecepcion)}
												</Muted>
					...
					<ScrollArea className="flex-1 pr-3">
						<section className="flex flex-col gap-4">
							{aeropuertos.filter((aeropuerto) =>
								...
								aeropuertos
									.filter((aeropuerto) =>
										aeropuerto.ubicacion.ciudadAbreviada
											.toLowerCase()
										...
										>
											<CardHeader>
												<Large>
													...
													{aeropuerto.ubicacion.ciudadAbreviada.toUpperCase()})
												</Large>
											</CardHeader>
											<CardContent>
											...
											<div className="absolute right-0 top-0 bottom-0 flex flex-col justify-center items-center gap-1 w-[59px]">
												<Button variant={"outline"} className="w-8 h-8 p-0" onClick={(e)=>{
                                                    e.preventDefault()
                                                    onClicks.onClickLocation(aeropuerto)
                                                }}>
								...
								<section className="flex flex-col gap-2">
									<Popover>
										...
										<PopoverContent className="w-auto p-0">
											<Calendar
												mode="single"
												selected={flyDateStart}
												onSelect={setFlyDateStart}
												initialFocus
											/>
										</PopoverContent>
									</Popover>
