			...
			>
				...
				<CardContent className="w-full flex flex-col items-center space-y-6 *:w-full overflow-hidden">
					...
					{selectedOperation === Operacion.Envios && (
						<Envios envios={envios} onClicks={onClicksEnvio} tiempoActual={tiempoActual} ubicaciones={ubicaciones} />
					)}
					...
					{selectedOperation === Operacion.Vuelos && (
						<Vuelos vuelos={vuelos} onClick={onClickVuelo} tiempoActual={tiempoActual} ubicaciones={ubicaciones} />
					)}
				</CardContent>
			</Card>
		</>
...
function Envios({
	...
	const filteredItems = useMemo(() => {
		...
		if (tiempoActual) {
			filteredEnvios = filteredEnvios?.filter(
				(envio) => envio.fechaRecepcion.getTime() <= tiempoActual.getTime() && envio.fechaLimiteEntrega.getTime() >= tiempoActual.getTime()
			);
		}
		...
		if (rangoCapacidadFilter) {
			filteredEnvios = filteredEnvios?.filter((envio) => envio.cantidadPaquetes >= minCapacidad && envio.cantidadPaquetes <= maxCapacidad);
		}
		if (receptionDateStart && receptionDateEnd) {
			filteredEnvios = filteredEnvios?.filter(
				(envio) =>
					envio.fechaRecepcion.getTime() >= receptionDateStart.getTime() && envio.fechaRecepcion.getTime() <= receptionDateEnd.getTime()
			);
		}
		if (hasSearchFilter) {
	...
	const renderCard = useCallback(
		(envio: Envio) => (
			<Card key={envio.id} className="p-3 *:p-0 hover:bg-gray-100 cursor-pointer" onClick={() => onClicks.onClickInfo(envio)}>
				<CardHeader>
		...
		return (
			<>
				<section className="flex flex-row justify-between gap-4 w-full">
					<Input placeholder="Buscar envío..." value={search} onChange={(e) => onSearchChange(e.target.value)} />
					<Popover>
...
function Aeropuertos({
	...
	const renderFilters = useCallback(() => {
		return (
			<>
				<section className="flex flex-row justify-between gap-4 w-full">
					<Input placeholder="Buscar aeropuerto..." value={search} onChange={(e) => onSearchChange(e.target.value)} />
					<Popover>
						<PopoverTrigger asChild>
							<Button variant="secondary">
								<ListFilter className="mr-2 h-4 w-4" /> Filtros
							...
							{continentes.map((continente) => (
								<div className="ml-4 flex items-center space-x-2" key={continente}>
									<Checkbox
										...
										onCheckedChange={(checked) => {
											setContinentesFilter((prev) => (checked ? [...prev, continente] : prev.filter((c) => c !== continente)));
										}}
									/>
									<Label htmlFor={continente}>{continente}</Label>
								</div>
...
function Vuelos({
	...
	const renderFilters = useCallback(() => {
		return (
			<>
				<section className="flex flex-row justify-between gap-4 w-full">
					<Input placeholder="Buscar vuelo..." value={search} onChange={(e) => onSearchChange(e.target.value)} />
					<Popover>
						<PopoverTrigger asChild>
							<Button variant="secondary">
								<ListFilter className="mr-2 h-4 w-4" /> Filtros
