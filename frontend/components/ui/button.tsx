...
const buttonVariants = cva(
  "gap-2 inline-flex items-center justify-center whitespace-nowrap rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50",
  {
    variants: {
      variant: {
        default: "bg-mainRed text-primary-foreground hover:bg-mainRed/90",
        defaultBlack: "bg-primary text-primary-foreground hover:bg-primary/90",
        destructive:
  ...
  ({ className, variant, size, asChild = false, isLoading = false, ...props }, ref) => {
    ...
    return (
      <Comp
        ...
        {...props}
      >
        {isLoading && <Loader2 className="animate-spin h-5 w-5 mr-1"/>}
        {props.children}
      </Comp>
    )
  }
