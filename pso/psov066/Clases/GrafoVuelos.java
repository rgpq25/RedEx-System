...
public class GrafoVuelos {
    ...
    private boolean esRutaUnica(PlanRuta ruta, RutasAlmacenadas rutasGeneradas, int nuevaPosicion) {
        String codigo = ruta.getCodigo();
        if (rutasGeneradas.obtenerRutaPorCodigo(codigo) != null) {
            return false;
        } else {
            rutasGeneradas.agregarRuta(nuevaPosicion, ruta);
            return true;
        }
    }
    ...
    private PlanRuta buscarRutaAleatoriaDFS(Ubicacion actual, Ubicacion destino, Date fechaHoraActual,
            ...
            RutasAlmacenadas rutasGeneradas, int nuevaPosicion) {

        if (actual.getId().equals(destino.getId())) {
            if (esRutaUnica(rutaActual, rutasGeneradas, nuevaPosicion)) {
                ArrayList<Vuelo> vuelosClonados = new ArrayList<>(rutaActual.getVuelos());
                PlanRuta nuevaRuta = new PlanRuta();
                nuevaRuta.setVuelos(vuelosClonados);
                return nuevaRuta;
        ...
        for (Vuelo vuelo : vuelosPosibles) {
            if (fechaHoraActual.before(vuelo.getFecha_salida()) &&
                    !aeropuertosVisitados.contains(vuelo.getPlan_vuelo().getCiudadDestino().getId())) {
                // Agregar vuelo y continuar la búsqueda
                rutaActual.getVuelos().add(vuelo);
                aeropuertosVisitados.add(vuelo.getPlan_vuelo().getCiudadDestino().getId());
                ...
                // Backtracking
                rutaActual.getVuelos().remove(rutaActual.getVuelos().size() - 1);
                aeropuertosVisitados.remove(vuelo.getPlan_vuelo().getCiudadDestino().getId());
            }
        }
    ...
    public PlanRuta generarRutasPaquete(Paquete paquete,
            RutasAlmacenadas rutasGeneradas, int nuevaPosicion) {
        ...
        PlanRuta rutaEncontrada = buscarRutaAleatoriaDFS(paquete.getCiudadOrigen(), paquete.getCiudadDestino(),
                ...
                paquete, rutasGeneradas, nuevaPosicion);
