...
public class Funciones {
    ...
    public void saveRutesTxt(HashMap<String, ArrayList<PlanRuta>> rutas, String outputPath, int maxRowsPerFile) {
        int rowCount = 0; // Counter for the number of rows written
        int fileCount = 1; // Counter for the number of files created
        File csvFile = new File(outputPath + "/rutas_" + fileCount + ".txt"); // Initial file
        ...
        try {
            ...
            for (Map.Entry<String, ArrayList<PlanRuta>> entry : rutas.entrySet()) {
                ...
                for (PlanRuta planRuta : value) {
                    //out.println(planRuta.toString());
                    //iterate each vuelo
                    out.print("R"+contadorRutas);
                    for (Vuelo vuelo : planRuta.getVuelos()) {
                        out.print(",");
                        out.print(vuelo.getId());
                    }
                    contadorRutas++;  
                    //if last planRuta
                    if(contadorRutas != value.size()){
                        out.print(",");
                    }
                }
                //erase last ","
                ...
                rowCount++;
                out.println();
                // If maximum rows per file is reached, close current file and create a new one
                if (rowCount >= maxRowsPerFile) {
                    out.close();
                    rowCount = 0; // Reset row count for new file
                    fileCount++; // Increment file count
                    csvFile = new File(outputPath + "/rutas_" + fileCount + ".txt");
                    out = new PrintWriter(csvFile);
                }
            }
    ...
    public HashMap<Integer, ArrayList<PlanRuta>> getRutasValidasPorPaquete(ArrayList<Paquete> paquetes, HashMap<String, ArrayList<PlanRuta>> rutas){
        HashMap<Integer, ArrayList<PlanRuta>> rutasValidasPorPaquete = new HashMap<Integer, ArrayList<PlanRuta>>();
        //iterate each paquete
        for (Paquete paquete : paquetes) {
            ArrayList<PlanRuta> rutasValidas = new ArrayList<PlanRuta>();
            String ciudadOrigen = paquete.getCiudadOrigen().getId();
            String ciudadDestino = paquete.getCiudadDestino().getId();
            String cadena = ciudadOrigen + "-" + ciudadDestino;
            ArrayList<PlanRuta> planRutas = rutas.get(cadena);
            if(planRutas == null){
                System.out.println("No hay rutas para el paquete:" + paquete.getId());
                System.out.println(paquete);
                System.out.println("No hay rutas para la cadena: " + cadena);
                return null;
            } 
            //iterate planRutas
            ...
            rutasValidasPorPaquete.put(paquete.getId(), rutasValidas);
        }
    ...
    public void imprimirTodasLasRutas(HashMap<String, ArrayList<PlanRuta>> todasLasRutas){
        for (Map.Entry<String, ArrayList<PlanRuta>> entry : todasLasRutas.entrySet()) {
            ...
            for (PlanRuta planRuta : value) {
                ...
                /*if(planRuta.getVuelos() == null || planRuta.getVuelos().size() == 0){
        ...
        try {
            ...
            while (scanner.hasNextLine()) {
                ...
                if(paquete.getCiudadOrigen() == null){
                    System.out.println("No se encontro la ciudad origen: " + parts[1].trim());
                }
                if(paquete.getCiudadDestino() == null){
                    System.out.println("No se encontro la ciudad destino: " + parts[2].trim());
                }
