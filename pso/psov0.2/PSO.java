...
public class PSO {
    ...
    static double calcularCosto(Paquete paquete, Ruta ruta, List<Ruta> rutas) {
        ...
        if (!ruta.vuelos.get(0).origen.codigo.equals(paquete.origen.codigo) ||
                !ruta.vuelos.get(ruta.vuelos.size() - 1).destino.codigo.equals(paquete.destino.codigo))
            return 1000;

        try {
            //long tiempoRecepcion = Paquete.FORMATO_FECHA.parse(paquete.fechaRecepcion).getTime();
            long tiempoRecepcion = new SimpleDateFormat(formato).parse(paquete.fechaRecepcion).getTime();
