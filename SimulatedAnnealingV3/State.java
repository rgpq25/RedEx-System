...
public class State {
    ...
    public State generateNeighbours(Vuelo[] listaVuelos, Paquete[] listaPaquetes){
        ...
        neighbour.paquetes = nei_paquete;
        ...
        for(int i = 0; i < currentPlan.length; i++){
            currentPlan[i] = randomPackage.getListaVuelos()[i];
        }
        ...
        if(chosenMove == 0){
            ...
            int conteoVuelos = 0;
            for(int i = 0; i < currentPlan.length; i++){
                // System.out.println("Trying " + randomIndex);
                if(currentPlan[i].getId() == listaVuelos[randomIndex].getId()){
                    randomIndex = (int) (Math.random() * listaVuelos.length);
                    conteoVuelos++;

                    if(conteoVuelos == listaVuelos.length){
                        System.out.println("No more flights to add");
                        return neighbour;
                    }
                    i = 0;
                }
