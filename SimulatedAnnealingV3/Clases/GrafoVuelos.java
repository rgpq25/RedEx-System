...
public class GrafoVuelos {
    ...
    public void agregarVuelo(Vuelo vuelo) {
        if (!grafo.containsKey(vuelo.getPlan_vuelo().getId_ubicacion_origen())) {
            grafo.put(vuelo.getPlan_vuelo().getId_ubicacion_origen(), new ArrayList<>());
        }
        grafo.get(vuelo.getPlan_vuelo().getId_ubicacion_origen()).add(vuelo);
    }
    ...
    public void imprimirVuelos() {
        for (Map.Entry<String, List<Vuelo>> entrada : grafo.entrySet()) {
            ...
            for (Vuelo vuelo : entrada.getValue()) {
                System.out.println(
                        "  Vuelo a: " + vuelo.getPlan_vuelo().getId_ubicacion_destino() + ", ID: " + vuelo.getId());
            }
        }
    }
    ...
    private void buscarRutasDFS(String actual, String destino, Date fechaHoraActual, PlanRuta rutaActual,
        ...
        for (Vuelo vuelo : grafo.get(actual)) {
            if (fechaHoraActual.before(vuelo.getFecha_llegada())
                    && !aeropuertosVisitados.contains(vuelo.getPlan_vuelo().getId_ubicacion_destino())) {
                rutaActual.getVuelos().add(vuelo);
                aeropuertosVisitados.add(actual);
                buscarRutasDFS(vuelo.getPlan_vuelo().getId_ubicacion_destino(), destino, vuelo.getFecha_llegada(),
